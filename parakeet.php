...
try { 
  ...
  if ($test_success) {
    ...
    echo " oops, cyber gremlins ate a cord (again) connection down...<br>";}

  $stmt = $conn->prepare("SELECT * FROM recipes WHERE `COL 2` = 'parakeet'");
  $stmt->execute();
  ...
  <?php foreach ($recipes as $recipe) : ?>
    <div class="recipe-card">
      <div class="productImage">
        <?php
        $image = isset($recipe['COL 6']) ? $recipe['COL 6'] : '001fpo.png';
        if ($recipe['COL 1'] === 'parakeet001') {
          $image = 'images/000fpo.png';
        } else {
          $image = 'images/' . $image;}
        ?>
        <img src="<?= $image ?>" alt="<?= $recipe['COL 5'] ?>">
      </div>
